...
export class CreateOrder extends React.Component<Props, CreateOrderState> {
    constructor(props: Props) {
        ...
        this.state = {
            ...
            makerAmount: '1',
            takerAmount: '1',
            maker: 'paste maker address here',
        };
    }
    public createOrderAsync = async (): Promise<SignedOrder> => {
        const { makerTokenSymbol, makerAmount, takerTokenSymbol, takerAmount, maker } = this.state;
        ...
        const makerAddress = maker != 'paste maker address here' ? maker.toLowerCase() : addresses[0];
        ...
        try {
            const signedOrder = await signatureUtils.ecSignOrderAsync(provider, order, addresses[0]);
            // Store the signed Order
            console.log(signedOrder.signature);
            const vrsSignature = signedOrder.signature.slice(0, -2);

            const signatureType = makerAddress == addresses[0] ? SignatureType.EIP712 : SignatureType.Wallet;
            const typedOrderSignature = await signatureUtils.convertToSignatureWithType(vrsSignature, signatureType);
            signedOrder.signature = typedOrderSignature;
            console.log(typedOrderSignature);
    ...
    public render(): React.ReactNode {
        ...
        const makerAddressRender = (
            <PanelBlockField label="Maker Address">
                <Field hasAddons={true}>
                    <Control isExpanded={true}>
                        <Input
                            onChange={(e: any) => this.orderMakerAddressChanged(e.target.value, TraderSide.Maker)}
    ...
    // leaving trader side, as might want to introduce a case for taking orders
    public orderMakerAddressChanged = (address: string, traderSide: TraderSide) => {
        this.setState(prevState => {
            return traderSide === TraderSide.Maker
                ? { ...prevState, maker: address }
                : { ...prevState, maker: address };
        });
    };
